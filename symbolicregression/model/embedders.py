    ...
    def encode(self, sequences: List[Sequence]) -> List[torch.Tensor]:
        ...
        for seq in sequences:
            ...
            for x, y in seq:
                ...
                toks = [*x_toks, *y_toks]
                seq_toks.append([self.env.equation_word2id[tok] for tok in toks])
            res.append(torch.LongTensor(seq_toks))
        return res

    def batch(self, seqs: List[torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        pad_id = self.env.equation_word2id["<PAD>"]
...
class NumericalEmbedder(Embedder):
    def __init__(self, params, env):
        ...
        self.mlp_layers.append(nn.Linear(size, hidden_size))
        for i in range(self.params.n_emb_layers-1):
            self.mlp_layers.append(nn.Linear(hidden_size, hidden_size))
        self.out_linear = nn.Linear(hidden_size, self.output_dim)
    ...
    def compress(
        ...
        for layer in self.mlp_layers: sequences_embeddings = self.activation_fn(layer(sequences_embeddings))
        sequences_embeddings = self.out_linear(sequences_embeddings)
        return sequences_embeddings

    def forward(self, sequences: List[Sequence]) -> Tuple[torch.Tensor, torch.Tensor]:
        sequences = self.encode(sequences)
        sequences, sequences_len = self.batch(sequences)
        sequences, sequences_len = to_cuda(sequences, sequences_len, use_cpu=self.out_linear.weight.device.type=="cpu")
    ...
    def forward_prior(self, sequences: List[Sequence]) -> Tuple[torch.Tensor, torch.Tensor]:
        sequences = self.encode_prior(sequences)
        sequences, sequences_len = self.batch(sequences)
        sequences, sequences_len = to_cuda(sequences, sequences_len, use_cpu=self.out_linear.weight.device.type=="cpu")
        sequences_embeddings = self.embed(sequences)
        sequences_embeddings = self.compress(sequences_embeddings)
    ...
    def encode_prior(self, sequences: List[Sequence]) -> List[torch.Tensor]:
        res = []
        for seq in sequences:
            seq_toks = []
            for x, y in seq:
                ...
                x_toks = [
                    *[
                        "<INPUT_PAD>"
                        for _ in range(
                            (self.params.max_input_dimension - 0)
                            * self.float_scalar_descriptor_len
                        )
                    ],
                ]
